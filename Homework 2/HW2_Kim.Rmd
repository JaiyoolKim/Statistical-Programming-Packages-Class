...
## From the above shown data set, we need to fill the NA value with corresponding the value located in ID column.
...
## Naturally, each cell data means (except for first column data) sensory data corresponding to each operator.
...
hist(Arranged.data[,2],main="Histogram of sensory data from 1st operator",xlab="sensory data from 1st operator") 
...
## We need to transform the given data set with the # row : 22 (# of obs), and # col : 2 (Year (1900=0 format), Long jump).

## First, we seperate the data by variables by Year, Long jump
...
plot.ts(Arranged.data2.ts,xlab="year",ylab="Long Jump Record",main="TS plot for Long Jump Record series",col=2,lwd=2)
...
## C.f) What if taking the 'hist' function with different breaks and xlimit ?
...
## Assign each (repeated) data set to the each kind of tomato and each density 'list' space (refer to the following R code)
...
boxplot(as.numeric(Arranged.data4$`Second Tomato obtained from 2nd density`),main="Boxplot for Second Tomato obtained from 2nd density")

boxplot(as.numeric(Arranged.data4$`Second Tomato obtained from 3rd density`),main="Boxplot for Second Tomato obtained from 3rd density")
...
## Statistic : range = plants$pH_Max - plants$pH_Min ( we will use this statistic value as Dependent Variable(Y) )
...
Foliage_Color <- plants.NA.removed$Foliage_Color  ## We use this factor or explanatory variable (X) for ANOVA
...
## ANOVA TABLE 
## From the above ANOVA table, the $H_0 : \beta_1=\beta_2=\beta_3=\beta_4=\beta_5 =0$ has been rejected based on the p-value : 0.001141 (based on significance level (alpha) = 0.05). It implies there is at least one of the Foliage colors has the significant relationship with plants_range
...
## (Foliage-ColorGreen and Plants_range), there are significant relationship under the p-value 0.0101, 0.0371  respectively. (based on significance level (alpha) = 0.05)
